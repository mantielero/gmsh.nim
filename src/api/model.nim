...
proc getEntities*( dim:int = -1):seq[DimTag] =
  ##[
  ...
  var ierr:cint
  var dimTags:ptr cint
  var n:uint
  gmshModelGetEntities( cast[ptr ptr cint](dimTags.unsafeAddr), n.unsafeAddr,
                        dim.cint, ierr.unsafeAddr )
  assert( ierr == 0, "error while adding a new model")  
  return getOutDimTags(dimTags, n)
...
proc addPhysicalGroup*[I1,I2,I3:SomeInteger](dim:I1, tags:seq[I2]; tag:I3 = -1 ):seq[GroupTag] {.discardable.} =
  ##[
  Add a physical group of dimension dim', grouping the model entities with
  tags tags'. Return the tag of the physical group, equal to tag' if tag'
  is positive, or a new tag if tag' < 0.  
  ]##
  var values = castToCint(tags)
  var ierr:cint
  let t = gmshModelAddPhysicalGroup( dim.cint,
          cast[ptr cint](values[0].unsafeAddr), values.len.uint,
          tag.cint,
          ierr.unsafeAddr )
  assert( ierr == 0, "error while removing current model")  
  return @[t.GroupTag]
...
proc setPhysicalName*(dim:int, tag:GroupTag, name:string) =
  ##[
...
proc getBoundary*(tags:seq[DimTag]; 
                  combined:bool = true, 
                  oriented:bool = true, 
                  recursive:bool = false  ):seq[DimTag] =
  #[
  ...
  var ierr:cint
  var tmp = flatten(tags) #newSeq[cint](tags.len)
  var outDimTags:ptr cint
  var outDimTagsN:uint 

  gmshModelGetBoundary( cast[ptr cint](tmp[0].unsafeAddr), tmp.len.uint,
          cast[ptr ptr cint](outDimTags.unsafeAddr), outDimTagsN.unsafeAddr,
...
proc getEntitiesInBoundingBox*(p1,p2:Point; dim:int = -1 ):seq[DimTag] =
  #[
  ...
  var ierr:cint
  #var tmp = flatten(tags) 
  var outDimTags:ptr cint
  var outDimTagsN:uint 

  #let x1 =   #p1.x.cdouble
  gmshModelGetEntitiesInBoundingBox( p1.x.cdouble, p1.y.cdouble, p1.z.cdouble,
          p2.x.cdouble, p2.y.cdouble, p2.z.cdouble,
          cast[ptr ptr cint](outDimTags.unsafeAddr), outDimTagsN.unsafeAddr,
          dim.cint, ierr.unsafeAddr )
  assert( ierr == 0, "error while adding a new model")    
  return getOutDimTags(outDimTags, outDimTagsN)
...
proc getBoundingBox*( tag:DimTag ):tuple[bbMin:Point,bbMax:Point] =
  ##[
...
proc removeEntities*(tags:seq[DimTag];recursive:bool = false) =
  ##[
  ...
  gmshModelRemoveEntities( cast[ptr cint](objects[0].unsafeAddr), objects.len.cuint,
          recursive.cint, ierr.unsafeAddr )
  assert( ierr == 0, "error while adding a new model")  
#[
